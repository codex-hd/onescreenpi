...
const SENSITIVE_APP_SUGGESTIONS = [
  "1Password",
  "Bitwarden",
  "KeePassXC",
  "LastPass",
...
export function PrivacySection() {
  ...
  const handleAssistantAccessToggle = (checked: boolean) => {
    handleSettingsChange({ assistantAccessEnabled: checked }, false);
  };
  ...
  const addSensitiveAppSuggestion = (value: string) => {
    const exists = settings.ignoredWindows.some(
      (windowValue) => windowValue.toLowerCase() === value.toLowerCase()
    );
    if (exists) return;
    handleIgnoredWindowsChange([...settings.ignoredWindows, value]);
  };

  return (
    <div className="space-y-5">
      <p className="text-muted-foreground text-sm mb-4">
        Decide what gets captured, how long it stays local, and whether AI can read it.
      </p>
              ...
              <div className="flex items-center space-x-2.5">
                ...
                <div>
                  <h3 className="text-sm font-medium text-foreground flex items-center gap-1.5">
                    Assistant access
                    <HelpTooltip text="When off, the AI assistant cannot search or summarize your captured history. Turn it on only when you want an assistant to read local memory." />
                  </h3>
                  <p className="text-xs text-muted-foreground">
                    Off by default for new installs. You stay in control of when AI can read your history.
                  </p>
                </div>
              </div>
              <Switch
                id="assistantAccessEnabled"
                checked={Boolean(settings.assistantAccessEnabled)}
                onCheckedChange={handleAssistantAccessToggle}
              />
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="px-3 py-2.5">
            <div className="flex items-center space-x-2.5">
              ...
              <div>
                <h3 className="text-sm font-medium text-foreground flex items-center gap-1.5">
                  Retention
                  <HelpTooltip text="Keep data only as long as you need it. Auto-delete removes older local history on a rolling schedule." />
                </h3>
                <p className="text-xs text-muted-foreground">
                  Plain-English retention controls for local history.
                </p>
              </div>
            </div>
            <RetentionSettings embedded />
          </CardContent>
        </Card>
      </div>
              ...
              <CardContent className="px-3 py-2.5">
                ...
                <div className="ml-[26px]">
                  ...
                  <div className="mt-2 flex flex-wrap gap-2">
                    {SENSITIVE_APP_SUGGESTIONS.map((suggestion) => {
                      const isSelected = settings.ignoredWindows.some(
                      ...
                      return (
                        <Button
                          ...
                          disabled={isSelected}
                          onClick={() => addSensitiveAppSuggestion(suggestion)}
                        >
                          {isSelected ? `${suggestion} added` : `ignore ${suggestion}`}
                        </Button>
                      );
                    })}
                  </div>
