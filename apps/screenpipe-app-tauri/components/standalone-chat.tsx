...
function buildSystemPrompt(): string {
  ...
  return `You are a helpful AI assistant that can search through the user's OneScreenPI data - their screen recordings, audio transcriptions, and UI interactions.
...
export function StandaloneChat({ className }: { className?: string } = {}) {
  ...
  const assistantAccessBlocked = isSettingsLoaded && !settings.assistantAccessEnabled;
      ...
      if (piStartInFlightRef.current) {
        ...
        try {
          ...
          if (result.status === "ok" && result.data.running) {
            ...
            toast({ title: "Failed to start OneScreenPI Cloud", description: result.status === "error" ? result.error : "Unknown error", variant: "destructive" });
            return;
          }
        } catch (e) {
          toast({ title: "Failed to start OneScreenPI Cloud", description: String(e), variant: "destructive" });
          return;
        } finally {
          setPiStarting(false);
  ...
  const exportChatAsMarkdownFile = async () => {
    ...
    try {
      const filePath = await saveDialog({
        ...
        defaultPath: `onescreenpi-chat-${new Date().toISOString().replace(/[:.]/g, "-").slice(0, 19)}.md`,
      });
      if (filePath) {
      ...
      {assistantAccessBlocked ? (
        <div className="flex flex-1 items-center justify-center p-6">
          <div className="w-full max-w-md rounded-2xl border border-border bg-card p-6 text-center shadow-sm">
            ...
            <h2 className="text-lg font-semibold text-foreground">assistant access is off</h2>
            <p className="mt-2 text-sm text-muted-foreground">
              The AI assistant cannot read captured history until you enable it in Privacy settings.
            </p>
            <div className="mt-4 flex justify-center">
              <Button type="button" onClick={() => openSettingsWindow("privacy")}>
