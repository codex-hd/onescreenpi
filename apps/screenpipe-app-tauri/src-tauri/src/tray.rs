...
fn build_recording_controls_submenu(
    app: &AppHandle,
    settings: &SettingsStore,
    status: RecordingStatus,
) -> Result<tauri::menu::Submenu<Wry>> {
    let mut builder = SubmenuBuilder::new(app, "Capture controls");
    match crate::capture_control::get_capture_mode(settings) {
        crate::capture_control::CaptureMode::Paused => {
            builder = builder
                .item(&MenuItemBuilder::with_id("resume_capture", "Resume capture now").build(app)?)
                .item(&MenuItemBuilder::with_id("stop_capture", "Turn capture off").build(app)?);
        }
        crate::capture_control::CaptureMode::Stopped => {
            builder = builder
                .item(&MenuItemBuilder::with_id("start_capture", "Turn capture on").build(app)?);
        }
        crate::capture_control::CaptureMode::Running => {
            if matches!(status, RecordingStatus::Recording | RecordingStatus::Starting) {
                builder = builder
                    .item(&MenuItemBuilder::with_id("pause_capture_30m", "Pause for 30 minutes").build(app)?)
                    .item(&MenuItemBuilder::with_id("pause_capture_1h", "Pause for 1 hour").build(app)?)
                    .item(&MenuItemBuilder::with_id("pause_capture_rest_day", "Pause for rest of day").build(app)?)
...
fn build_quick_delete_submenu(app: &AppHandle) -> Result<tauri::menu::Submenu<Wry>> {
    SubmenuBuilder::new(app, "Quick delete")
        .item(&MenuItemBuilder::with_id("delete_last_5m", "Delete last 5 minutes").build(app)?)
        .item(&MenuItemBuilder::with_id("delete_today", "Delete today").build(app)?)
        .item(&MenuItemBuilder::with_id("delete_all_history", "Delete all history").build(app)?)
        .build()
        .map_err(Into::into)
}
...
fn confirm_and_delete(
    ...
    scope: crate::capture_control::QuickDeleteScope,
    title: &'static str,
    message: &'static str,
) {
    ...
    app.dialog()
        ...
        .show(move |confirmed| {
            ...
            tauri::async_runtime::spawn(async move {
                if let Err(error) = crate::capture_control::quick_delete(&app_inner, scope).await {
                    error!("quick delete failed: {}", error);
                }
            });
        });
}
...
fn create_dynamic_menu(
    ...
    if matches!(
        effective_status,
        RecordingStatus::Recording | RecordingStatus::Starting | RecordingStatus::Paused
    )
    {
        menu_builder = menu_builder.item(
    ...
    let status_label = if effective_status == RecordingStatus::Paused {
        if let Some(until) = crate::capture_control::get_capture_paused_until(&settings) {
            format!(
                "◐ Paused until {}",
    ...
    menu_builder = menu_builder.item(
        &MenuItemBuilder::with_id("recording_status", status_label)
            .enabled(false)
            .build(app)?,
    );

    if matches!(effective_status, RecordingStatus::Recording | RecordingStatus::Starting)
    {
    ...
    if !is_tray_item_hidden("tray_recording_controls") {
        menu_builder = menu_builder
            .item(&PredefinedMenuItem::separator(app)?)
            .item(&build_recording_controls_submenu(app, &settings, effective_status)?)
            .item(&build_quick_delete_submenu(app)?);
    }
...
fn handle_menu_event(app_handle: &AppHandle, event: tauri::menu::MenuEvent) {
    match event.id().as_ref() {
        ...
        "start_recording" | "toggle_recording" | "start_capture" => {
            set_optimistic_status(RecordingStatus::Starting);
            let app = app_handle.clone();
            tauri::async_runtime::spawn(async move {
                if let Err(error) = crate::capture_control::start_capture(&app).await {
                    error!("start capture failed: {}", error);
                }
            });
        ...
        "stop_recording" | "stop_capture" => {
            set_optimistic_status(RecordingStatus::Stopped);
            let app = app_handle.clone();
            tauri::async_runtime::spawn(async move {
                if let Err(error) = crate::capture_control::stop_capture(&app).await {
                    error!("stop capture failed: {}", error);
                }
            });
            let app2 = app_handle.clone();
            let _ = app_handle.run_on_main_thread(move || {
        ...
        "pause_capture_30m" | "pause_capture_1h" | "pause_capture_rest_day" => {
            let duration_minutes = match event.id().as_ref() {
                "pause_capture_30m" => 30,
                "pause_capture_1h" => 60,
                _ => 0,
            };
            ...
            let app = app_handle.clone();
            tauri::async_runtime::spawn(async move {
                if let Err(error) = crate::capture_control::capture_pause(app.clone(), duration_minutes).await {
                    error!("pause capture failed: {}", error);
                }
            });
            let app2 = app_handle.clone();
            let _ = app_handle.run_on_main_thread(move || {
        ...
        "resume_capture" => {
            ...
            let app = app_handle.clone();
            tauri::async_runtime::spawn(async move {
                if let Err(error) = crate::capture_control::resume_capture(&app).await {
                    error!("resume capture failed: {}", error);
                }
            });
            let app2 = app_handle.clone();
            let _ = app_handle.run_on_main_thread(move || {
        ...
        "delete_last_5m" => confirm_and_delete(
            app_handle,
            crate::capture_control::QuickDeleteScope::Last5Minutes,
            "Delete last 5 minutes",
            "This will permanently remove the last 5 minutes of captured history.",
        ),
        "delete_today" => confirm_and_delete(
            app_handle,
            crate::capture_control::QuickDeleteScope::Today,
            "Delete today",
            "This will permanently remove everything captured today.",
        ),
        "delete_all_history" => confirm_and_delete(
            app_handle,
            crate::capture_control::QuickDeleteScope::AllHistory,
            "Delete all history",
            "This will permanently remove all captured history on this device.",
        ),
        "lock_vault" => {
