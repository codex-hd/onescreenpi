...
pub async fn start_health_check(app: tauri::AppHandle) -> Result<()> {
    ...
    tokio::spawn(async move {
        loop {
            ...
            let status = if let Ok(Some(settings)) = crate::store::SettingsStore::get(&app) {
                crate::capture_control::recording_status_for_settings(&settings, status)
            } else {
                status
            };
            ...
            if current_status != last_status || theme != last_theme {
                ...
                let icon_path = if current_status == "paused" {
                    if theme == Mode::Light {
                        "assets/screenpipe-logo-tray-beta-black.png"
                    } else {
                        "assets/screenpipe-logo-tray-beta-white.png"
                    }
                } else if is_unhealthy_icon(current_status) {
                    if theme == Mode::Light {
            ...
            if let Ok(ref health) = health_result {
                if health.drm_content_paused && !drm_stopped {
                    ...
                    if let Some(recording_state) = app.try_state::<crate::recording::RecordingState>() {
                        let _ = crate::recording::stop_screenpipe_internal(&*recording_state).await;
                    }
                    drm_stopped = true;
            ...
            if drm_stopped {
                ...
                if stop_elapsed < Duration::from_secs(5) {
                    ...
                    if should_resume {
                        ...
                        if let Some(recording_state) = app.try_state::<crate::recording::RecordingState>() {
                            let _ = crate::recording::spawn_screenpipe_internal(&*recording_state, app.clone(), None).await;
                        }
                        drm_stopped = false;
                        drm_stop_time = None;
