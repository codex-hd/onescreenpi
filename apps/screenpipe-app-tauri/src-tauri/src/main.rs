...
async fn main() {
    ...
    let app = app.manage(recording_state)
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            calendar::calendar_get_events,
            calendar::calendar_get_current_meeting,
            capture_control::capture_start,
            capture_control::capture_stop,
            capture_control::capture_pause,
            capture_control::capture_resume,
            capture_control::capture_quick_delete,
            // ICS Calendar commands
            ics_calendar::ics_calendar_get_entries,
            ics_calendar::ics_calendar_save_entries,
            ics_calendar::ics_calendar_test_url,
        ...
        .setup(move |app| {
            ...
            if let Err(e) = capture_control::initialize(&app_handle) {
                warn!("failed to initialize capture control state: {}", e);
            }
            let store = store::SettingsStore::get(&app.handle())
                .unwrap_or_default()
                .unwrap_or(store);
            let should_start_capture = capture_control::should_start_on_launch(&store);
            ...
            if should_start_capture {
                ...
                info!(
                    "capture startup skipped due to user mode '{}'",
                    store.capture_mode
                );
            }
