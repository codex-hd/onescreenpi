...
// Windows native OCR benchmark (Windows only)
#[cfg(target_os = "windows")]
fn bench_windows_native_ocr(c: &mut Criterion) {
    let image = load_test_image(); // Load the image once
    let mut group = c.benchmark_group("Windows Native OCR");
    group.sample_size(10);
    group.measurement_time(Duration::from_secs(10));
...
// Tesseract OCR benchmark on Windows so we can compare native latency/quality.
#[cfg(target_os = "windows")]
fn bench_windows_tesseract_ocr(c: &mut Criterion) {
    let image = load_test_image();
    let mut group = c.benchmark_group("Windows Tesseract OCR");
    group.sample_size(10);
    group.measurement_time(Duration::from_secs(10));
    ...
    group.bench_function(BenchmarkId::new("Performance and Accuracy", ""), |b| {
        b.iter_custom(|iters| {
            ...
            println!(
                "Average Accuracy (Tesseract on Windows): {:.2}",
                total_accuracy / iters as f32
            );
            total_duration
        })
    });
...
criterion_group!(
    benches,
    bench_windows_native_ocr,
    bench_windows_tesseract_ocr
);
